...
for k in range(1,11):
	...
	#capture n:		
	data = pd.read_csv('capture_n'+str(k)+'_sending_24_bytes_without_ltl.csv')
	...
	if k==1: #affichage display pour k==1
		...
		plt.tight_layout()
		# Show the plot
		plt.savefig("fig2_without_ltl_with_sensors_energy.png")
		plt.show()
...
with open('energy_sending_24_bytes_without_ltl.csv', 'w', newline='') as file24bytes:
   ...
   # écrire une ligne dans le fichier:
    ...
    ecrivain.writerow(['legend_graph_2','delta E measure sensor:(mJ)',deltaElegend_measure,'delta E build_light:(mJ)',"nan",'delta E send lora:(mJ)',deltaElegend_lssend])
