...
#define TX_INTERVAL 5 // Interval between transmissions (in seconds)
...
/*
  ...
  //*/
//#define DATA_SIZE 120 //maxsize is
...
*/
//CURRENTLY NON USED
//#define SMART_CONTRACT_SIZE 13 //max_size is 12
//char my_smart_contract_name[SMART_CONTRACT_SIZE] = {0x69, 0x6f, 0x74, 0x62, 0x69, 0x6f, 0x74, 0x62, 0x69, 0x6f, 0x74, 0x62, 0x69};
...
void setup() {
  ...
  pinMode(LS_GPS_ENABLE, OUTPUT); // LS_GPS_ENABLE = 32: PB11
  digitalWrite(LS_GPS_ENABLE, LOW);
  pinMode(LS_GPS_V_BCKP, OUTPUT);
  digitalWrite(LS_GPS_V_BCKP, HIGH); // LS_GPS_V_BCKP = 37: PA6

  digitalWrite(LS_VERSION_ENABLE, LOW); // LS_VERSION_ENABLE = 40: PC6
...
Serial3.println("1");
  ...
  int error = build_light_payload(private_key, public_key_compressed, my_message, my_protobuffer, &protobuffer_length);
    Serial3.println("5");
  // Seria3l.print("Mon proto est long de : "); Serial.println(protobuffer_length);
